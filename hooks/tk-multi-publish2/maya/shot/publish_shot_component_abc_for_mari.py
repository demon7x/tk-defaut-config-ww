...
class MayaSessionComponentAembicForMariPublishPlugin(HookBaseClass):
    """
    ...
    def settings(self):
        ...
        base_settings = super(MayaSessionComponentAembicForMariPublishPlugin, self).settings or {}
    ...
    def item_filters(self):
        ...
        ["maya.*", "file.maya"]
        """
        return ["maya.session.shot.component.abc_mari"]
    ...
    def accept(self, settings, item):
        ...
        if not item.parent.parent.context.step['name'] == "model":
            self.logger.debug(
                "publish session with model."
            )
            accepted = False
    ...
    def validate(self, settings, item):
        ...
        return super(MayaSessionComponentAembicForMariPublishPlugin, self).validate(
            settings, item)
    ...
    def publish(self, settings, item):
        ...
        alembic_args.append("-root %s" % item.properties['name']+"_merged")
        ...
        try:
            ...
            meshs = []
            for mat in mtl_tag.keys():
                if len(mtl_tag[mat]) > 1:
                    mesh,pu = cmds.polyUnite(mtl_tag[mat],muv=1,name=mat)
    
                elif len(mtl_tag[mat]) == 1:
                    cmds.parent(cmds.listRelatives(mtl_tag[mat][0],p=1),w=1)
                    mesh = cmds.rename(cmds.listRelatives(mtl_tag[mat][0],p=1),mat)
                meshs.append(mesh)
        ...
        super(MayaSessionComponentAembicForMariPublishPlugin, self).publish(settings, item)
