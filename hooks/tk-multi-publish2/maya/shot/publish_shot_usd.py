...
class MayaSessionShotUSDPublishPlugin(HookBaseClass):
    """
    ...
    def settings(self):
        ...
        base_settings = super(MayaSessionShotUSDPublishPlugin, self).settings or {}
    ...
    def validate(self, settings, item):
        ...
        return super(MayaSessionShotUSDPublishPlugin, self).validate(
            settings, item)
    ...
    def publish(self, settings, item):
        ...
        try:
            ...
            shot_stage = Usd.Stage.Open(root_layer)
            shot_stage.SetStartTimeCode(item.properties['s_f'])
            shot_stage.SetEndTimeCode(item.properties['e_f'])
            UsdGeom.SetStageUpAxis(shot_stage, UsdGeom.Tokens.y)

            


            for child_item in item.children:
                ref_prim = UsdGeom.Xform.Define(shot_stage,"/%s"%child_item.properties['namespace']).GetPrim()
                ref_prim.GetReferences().AddReference(child_item.properties['path'])
                xformAPI = UsdGeom.XformCommonAPI(ref_prim)
                xformAPI.SetTranslate(child_item.properties['translate'])
                xformAPI.SetRotate(child_item.properties['rotate'])
                xformAPI.SetScale(child_item.properties['scale'])
        ...
        super(MayaSessionShotUSDPublishPlugin, self).publish(settings, item)
