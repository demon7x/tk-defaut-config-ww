...
class MayaSessionShotScenegraphXMLPublishPlugin(HookBaseClass):
    """
    ...
    def settings(self):
        ...
        base_settings = super(MayaSessionShotScenegraphXMLPublishPlugin, self).settings or {}
    ...
    def item_filters(self):
        ...
        ["maya.*", "file.maya"]
        """
        return ["maya.session.shot.scenegraphxml"]
    ...
    def validate(self, settings, item):
        ...
        return super(MayaSessionShotScenegraphXMLPublishPlugin, self).validate(
            settings, item)
    ...
    def publish(self, settings, item):
        ...
        assembly = item.properties['name']
        try:

            
            root = sgxml.ScenegraphRoot()
            component_list = []
            #maya2scenegraphXML.setAssembly([assembly])

            for child_item in item.children:
                abc_path = child_item.properties['path']
                component = child_item.properties['name']
                matrix = child_item.properties['matrix']
                component_ref = sgxml.Reference(name=component, refType='abc', refFile=abc_path)
                component_ref.setXform(value=matrix)
                component_list.append(component_ref)
            root.setInstanceList(component_list)

            start_frame, end_frame = _find_scene_animation_range()
            if start_frame and end_frame:
                fileDir, fileStem = os.path.split(publish_path)
                if fileStem.endswith('.xml'):
                    fileStem = fileStem[:-4]
                root.channelData = sgxml.ChannelData(start_frame, end_frame, fileStem)
            else:
                root.channelData = sgxml.ChannelData(1001, 1001)
        ...
        super(MayaSessionShotScenegraphXMLPublishPlugin, self).publish(settings, item)
